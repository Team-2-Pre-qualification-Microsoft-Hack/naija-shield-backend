...
public sealed class ThreatScoringService : IThreatScoringService
{
    ...
    private const string SystemPrompt = """
        ...
        Response: {"riskScore":97,"classification":"OTP_PHISH","confidence":0.98,"explanation":"Requests OTP with urgency under threat of account suspension impersonating GTBank","recommendedAction":"BLOCK"}

        Message (SMS): "Hi Mum, I changed my number. This is John. I'm in trouble — please send N50,000 urgent."
        Response: {"riskScore":88,"classification":"SOCIAL_ENGINEERING","confidence":0.93,"explanation":"Classic family-impersonation scam requesting urgent money transfer from a new number","recommendedAction":"BLOCK"}

        Message (SMS): "Your GTBank account is restricted. Verify your identity at http://gtb-secure.ng immediately."
        Response: {"riskScore":95,"classification":"IMPERSONATION","confidence":0.96,"explanation":"Bank impersonation with suspicious phishing URL requesting identity verification","recommendedAction":"BLOCK"}

        Message (SMS): "Your OTP to access your account is 123456. Valid for 5 minutes. Do not share this code with anyone."
        Response: {"riskScore":8,"classification":"SAFE","confidence":0.97,"explanation":"Standard OTP delivery from a verified source with an explicit do-not-share warning","recommendedAction":"ALLOW"}

        Message (Voice transcript): "Oga send me the OTP now now, my boss don say make I collect am from you."
        Response: {"riskScore":91,"classification":"OTP_PHISH","confidence":0.89,"explanation":"Urgent Pidgin-English request for OTP using social pressure and authority reference","recommendedAction":"BLOCK"}

        Message (SMS): "EFCC: Your BVN has been flagged for fraud. Call 08099887766 within 1 hour or be arrested."
        Response: {"riskScore":99,"classification":"VISHING","confidence":0.99,"explanation":"EFCC impersonation with arrest threat designed to extract personal information by phone","recommendedAction":"BLOCK"}

        Message (SMS): "Transfer of N5,000 to Chijioke Obi was successful. Balance: N42,300. Not you? Call 07001234567."
        Response: {"riskScore":12,"classification":"SAFE","confidence":0.94,"explanation":"Standard bank debit notification with a legitimate customer-care callback number","recommendedAction":"ALLOW"}
